...
### 7.2 LLM 클라이언트 (폴백 모드 지원)

개발 시 Gateway 없이도 동작하도록 폴백 모드 지원:
...
class GatewayLLMClient {
  constructor(gatewayUrl, serviceToken, tenantId, service) {
  ...
  async complete({ messages, system, model, maxTokens, cacheTtl }) {
    const response = await fetch(`${this.gatewayUrl}/api/v1/complete`, {
      method: 'POST',
      headers: {
...
export function createLLMClient(tenantId, service) {
  const useGateway = process.env.USE_LLM_GATEWAY === 'true';
  ...
  if (useGateway) {
    ...
    // 개발 모드: 직접 호출
    console.warn('[LLM] Using direct API call (development mode)');
...
class DirectLLMClient(LLMClient):
    """직접 Claude API 호출 (개발용)"""
    
    def __init__(self):
        self.client = Anthropic()
    
    async def complete(self, messages, system=None, model=None, max_tokens=1000, **kwargs):
...
class GatewayLLMClient(LLMClient):
    ...
    def __init__(self, gateway_url, service_token, tenant_id, service):
        self.gateway_url = gateway_url
        self.service_token = service_token
        self.tenant_id = tenant_id
        self.service = service
    
    async def complete(self, messages, system=None, model=None, max_tokens=1000, cache_ttl=None, **kwargs):
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self.gateway_url}/api/v1/complete",
                headers={
...
def create_llm_client(tenant_id: str, service: str) -> LLMClient:
    use_gateway = os.getenv("USE_LLM_GATEWAY", "false").lower() == "true"
    ...
    if use_gateway:
        ...
        print("[LLM] Using direct API call (development mode)")
